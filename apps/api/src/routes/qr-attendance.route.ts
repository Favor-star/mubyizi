...
const qrAttendanceRoutes = new Hono<HonoInstanceContext>()
  ...
  .post(
    ...
    async (c) => {
      try {
        ...
        const expiresAt = new Date(`${targetDate}T23:59:59.999Z`);

        return c.json(
          apiSuccessResponse({ token, qrImageDataUrl, workplaceId, date: targetDate, expiresAt }, "QR code generated"),
          200
        );
      } catch (error) {
  ...
  .post(
    "/scan",
    requireAuth(),
    customValidator("json", scanQrSchema),
    describeRoute(scanQrDocs),
    async (c) => {
      try {
        const prisma = c.get("prisma");
        const currentUser = c.get("user")!;
